/**
 * Created by AbelRicardo on 3/8/2017.
 */
...
@Component({
  selector: 'app-cotizaciondecor',
  templateUrl: 'cotDecoracion.html',
  styleUrls: ['cotEstilos.css'],
  providers: [ ObInfoRestService ]
})

export class CotizarDecComponent {
// creamos un nuevo objeto de form y seteamos su tipo como complexform
  ...
  decor = ['Stands de Exposiciones',
    'Vidrieras Comerciales' ,
     'Publicidad Interior de Locales Comerciales',
     'Motivos Infantiles'];
...
// pasamos una instancia del FormBuilder a nuestro constructor
  ...
  return val ? this.decor.filter((s) => new RegExp(val, 'gi').test(s)) : this.decor;
}
  ...
  submitForm(value: any): void {
    /*console.log('Reactive FormData');
     console.log(value);*/
    this.enviarcorreo();
    this.router.navigate(['/services/decoracion']);
  }
